...
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		...
		for (Fish fish : fish) {
			if (fish.getCol() == c && fish.getRow() == r) {
				return false;
			}
		}
		for (Plant plant : plants) {
			if (plant.getCol() == c && plant.getRow() == r) {
				return false;
			}
		}
		return true;
	...
	public Model(Model other) {
		...
		this.landscape = new boolean[other.getRows()][other.getCols()];
		
		for (int row = 0; row < other.getRows(); row++) {
			for (int col = 0; col < other.getCols(); col++) {
				if (other.landscape[row][col] == ROCK) {
					this.landscape[row][col] = ROCK;
					
				} else {
					this.landscape[row][col] = WATER;
				}
			}
		}
			...
			this.fish = new ArrayList<Fish>();
				...
				for(Fish f: other.fish) {
					this.fish.add(new Fish(f));
		}
				
			this.plants = new ArrayList<Plant>();
			
				for(Plant p: other.plants) {
					this.plants.add(new Plant(p));
		}
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		...
		int fishCol = f.getCol();
		int fishRow = f.getRow();
		
		
		
		if (landscape[fishRow][fishCol + 1] == ROCK &&
				landscape[fishRow][fishCol - 1] == ROCK &&
				landscape[fishRow + 1][fishCol] == ROCK &&
				landscape[fishRow - 1][fishCol] ==  ROCK) {
	...
	public void turnFish() {
		for (Fish f : fish) {
			...
			if (f.getDirection() != Fish.LEFT && f.getDirection() != Fish.RIGHT && f.getDirection() != Fish.UP
					&& f.getDirection() != Fish.DOWN) {
		...
		for (Fish f: fish) {
			...
			if (f.isAlive()) {
				if (fishIsSurroundedByRocks(f)) {
					...
					for (Fish newF : fish) {
						if (newF.getCol() == f.getCol() && newF.getRow() == f.getRow() && newF != f) {
							f.fight(newF);
	...
	public void addPlant(Plant p) {
		...
		for (Plant newP : plants) {

			if ((newP != p && newP.getRow() == p.getRow() && newP.getCol() == p.getCol())) {
	...
	public void addFish (Fish f) {
		...
		for (Fish newF : fish) {
			...
			if ((newF != f && newF.getRow() == f.getRow() && newF.getCol() == f.getCol())) {
	...
	public ArrayList<Fish> getFish() {
		ArrayList<Fish> fishCopy = new ArrayList<Fish>(fish);
		return fishCopy;
